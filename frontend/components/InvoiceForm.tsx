    ...
    <>
      <form onSubmit={onSubmit} className="card" style={{ display: "grid", gap: 12 }}>
        <div>
          ...
          <div className="helper">Exact amount required; we generate a Solana Pay link with a unique reference.</div>
        </div>
        <div>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
          <button className="btn" type="button" onClick={() => { setAmount(""); setRecipient(""); setMemo(""); setLabel(""); setMessage(""); setEmail(""); setLink(null); }}>
            Clear
          </button>
      ...
      {link && (
        <div>
          <QRCard url={link.url} />
          <div className="badge" style={{ marginTop: 8 }}>Reference <code>{link.reference}</code></div>
        </div>
      )}
    </>
  );
